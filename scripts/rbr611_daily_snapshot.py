#!/usr/bin/env python3
"""
RBR-611 daily 09:30 PT snapshot (Jul 23, Day 13).

Day 13 — Thursday Jul 23, Phase 4 Day 3.
Evidence assembly continues; check if IT processed any Drata retirement tickets.
This is the third snapshot in the Phase 4 evidence window.

Per playbook §3.3, today is Phase 4 Day 3. Actions:
  - Capture fleet + pilot snapshot (same format as RBR-609).
  - Reconcile against RBR-609 (Jul 22) for day-over-day delta.
  ...
  - Emit Phase 4 Day 3 status: evidence assembly continues, 5 days to Jul 28 audit.
...
def main():
    ...
    if pilot_pct >= 90:
        ...
        gate = "ABORT (Phase 1 EOD Jul 11 report; no change at 09:30 PT Jul 23)"
    ...
    # ------------------------------------------------------------------
    # Residual hand-off (Phase 4 Day 3 — IT continues retirement)
    # ------------------------------------------------------------------
    touched_ids = set()
    ...
    snapshot = {
        ...
        "capturedBy": "secops",
        "capturedFor": "RBR-611 daily 09:30 PT snapshot — Day 13 (Phase 4 Day 3). Evidence assembly continues. 5 days to Jul 28 audit.",
        "scope": "fleet+pilot",
        ...
        "topIssues": [
            f"Day 13 — Phase 4 Day 3 evidence assembly. Cumulative cascade efficacy: {total_moved}/{total_cascade_targets} across 6 cascade days (unchanged). Device-side staleness is the bottleneck, not cascade architecture.",
            f"Fleet strict-compliant count {fleet_compliant}/{baseline_total} ({pct_compliant}%); delta vs Jul 22: agent {delta_with_agent:+d}, compliant {delta_compliant:+d}.",
        ...
        "dataReconciliation": {
            ...
            "note": f"Day-over-day delta vs RBR-609 (Jul 22). Phase 4 Day 3. {retirement_note}",
        },
        "cumulativeCascadeEfficacy": {
            ...
            "verdict": (
                ...
                "bottleneck, not cascade architecture. Phase 4 Day 3 focus: continue evidence assembly "
                "for agent-installed devices + monitor IT Drata retirement progress."
            ),
        },
        "residualHandOff": {
            "phase": "Phase 4 Day 3 — IT continues manual Drata retirement",
            ...
            "note": (
                ...
                f"Phase 4 Day 3: {retirement_note}"
            ),
        },
        "phase4EvidenceReadiness": {
            ...
            "recommendation": (
                ...
                "and manual Drata retirement for stale residuals. 5 days remain until Jul 28 Stage 1 audit."
            ),
        },
        "dayNCascadePlan": {
            "day": 3,
            "phase": "Phase 4 evidence assembly",
            "phaseDay": 13,
            "action": (
                "Phase 4 Day 3 — evidence assembly continues. Check if IT processed Drata "
                "retirement tickets (Drata index should shrink). SecOps continues A.8.1/A.8.7/A.8.8 "
                "evidence package assembly. No cascade outreach today (all cascade days exhausted)."
            ),
            "scheduledAt": now.isoformat().replace("+00:00", "Z"),
            "owner": "secops (evidence assembly) + IT (device retirement)",
            "nextDayActions": [
                "Jul 24 (Fri): Phase 4 Day 4 — continue evidence assembly; check retirement delta.",
                "Jul 24-25: Finalize evidence package for Jul 28 Stage 1 audit.",
                "Jul 25: Final evidence readiness gate before audit.",
            ],
        },
        "actionsTakenThisDay": [
            "09:30 PT: SecOps captured this RBR-611 Day 13 snapshot (fleet + pilot) via direct Drata API call -- Phase 4 Day 3.",
            "Reconciled day-over-day delta vs RBR-609 (Jul 22).",
            ...
            "Emitted Phase 4 Day 3 evidence readiness assessment (A.8.1, A.8.7, A.8.8).",
        ],
        "nextDayActions": [
            "Jul 24 (Fri): Phase 4 Day 4 — continue evidence assembly.",
            "Jul 24: Check delta on retired devices (Drata index should shrink if retirements processed).",
            "Jul 24-25: Finalize evidence package for Jul 28 Stage 1 audit.",
        ],
    }
    ...
    monitor["rollout"] = {
        "phase": f"Phase 4 — Evidence assembly (Day {13})",
        "owningIssueId": "eaa0c9d0-f44b-4b84-b92c-5a6b25e27549",
        "owningIssueIdentifier": "RBR-611",
    ...
    summary = {
        ...
        "phase": "Phase 4 evidence assembly Day 3 (Day 13)",
