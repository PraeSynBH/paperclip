#!/usr/bin/env python3
"""
RBR-606 daily 09:30 PT snapshot (Jul 21, Day 11).

Day 11 — Tuesday Jul 21, Phase 4 evidence collection kickoff.
Phase 4 (RBR-98) starts today: verify A.8.1/A.8.7/A.8.8 evidence in Drata.
This is the first snapshot in the Phase 4 evidence window.

Per playbook §3.3, today is Phase 4 Day 1. Actions:
  - Capture fleet + pilot snapshot (same format as RBR-559/RBR-599/RBR-603).
  - Reconcile against RBR-603 (Jul 20) for day-over-day delta.
  - Continue cumulative cascade efficacy readout.
  - Emit Phase 4 Day 1 status: IT takes residual hand-off cohort.
  - Flag any new heartbeats or agent installations since last check.
  - Emit Phase 4 evidence verification readiness for A.8.1/A.8.7/A.8.8.
...
def main():
    ...
    if pilot_pct >= 90:
        ...
        gate = "ABORT (Phase 1 EOD Jul 11 report; no change at 09:30 PT Jul 21)"

    outstanding_installs = sum(1 for d in devices if d.get("sourceType") != "AGENT")

    # ------------------------------------------------------------------
    # Data reconciliation vs RBR-603 (last snapshot, Jul 20)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Residual hand-off (Phase 4 Day 1 — IT takes cohort)
    # ------------------------------------------------------------------
    touched_ids = set()
    ...
    evidence_a8_1 = {
        ...
        "gaps": [
            ...
            "197 Unknown-OS orphans (RBR-108)",
        ],
    }
    evidence_a8_7 = {
    ...
    snapshot = {
        ...
        "capturedBy": "secops",
        "capturedFor": "RBR-606 daily 09:30 PT snapshot -- Day 11 (Phase 4 kickoff). Evidence verification window starts today. IT takes residual hand-off cohort for manual Drata retirement.",
        "scope": "fleet+pilot",
        ...
        "topIssues": [
            f"Day 11 — Phase 4 evidence verification kickoff. Cumulative cascade efficacy: {total_moved}/{total_cascade_targets} across 6 cascade days (unchanged). Device-side staleness is the bottleneck, not cascade architecture.",
            f"Fleet strict-compliant count {fleet_compliant}/{baseline_total} ({pct_compliant}%); delta vs Jul 20: agent {delta_with_agent:+d}, compliant {delta_compliant:+d}.",
            ...
            f"Residual hand-off cohort: {len(residual_targets)} silent devices not touched by any cascade day -- IT manual Drata retirement tickets begin today.",
            "197 Unknown-OS orphans unchanged -- RBR-108 still pending; needs Drata index rebuild.",
        ],
        "dataReconciliation": {
            "priorSnapshot": "2026-07-20 (RBR-603 daily 09:30 PT Day 10)",
            ...
            "note": "Day-over-day delta vs RBR-603 (Jul 20). Phase 4 kickoff day.",
        },
        "cumulativeCascadeEfficacy": {
            ...
            "verdict": (
                ...
                "bottleneck, not cascade architecture. Phase 4 focus: evidence verification for agent-installed "
                "devices + manual Drata retirement for stale residuals."
            ),
        },
        "residualHandOff": {
            "phase": "Phase 4 Day 1 — IT manual Drata retirement kickoff",
            ...
            "note": (
                ...
                "Phase 4 Day 1: IT begins manual Drata retirement tickets to clear stale entries."
            ),
        },
        "phase4EvidenceReadiness": {
            ...
            "blockingIssues": {
                ...
                "RBR-108": "197 Unknown-OS devices in Drata -- todo (not blocking evidence window)",
            },
            "fleetEnteringPhase4": {
                "withAgent": fleet_with_agent,
            ...
            "recommendation": (
                ...
                "and manual Drata retirement for stale residuals. Target: have evidence "
                "package ready by Jul 25 for Jul 28 Stage 1 audit."
            ),
        },
        "dayNCascadePlan": {
            "day": 1,
            "phase": "Phase 4 evidence verification",
            "phaseDay": 11,
            "action": (
                "Phase 4 Day 1 — evidence verification kickoff. IT begins manual Drata "
                "retirement tickets for residual hand-off cohort. SecOps initiates A.8.1/A.8.7/A.8.8 "
                "evidence package assembly. No cascade outreach today (all cascade days exhausted)."
            ),
            "scheduledAt": now.isoformat().replace("+00:00", "Z"),
            "owner": "secops (evidence verification) + IT (device retirement)",
            "nextDayActions": [
                "Jul 22 (Wed): Phase 4 Day 2 — continue evidence assembly; IT processes retirement tickets.",
                "Jul 22: Check delta on retired devices (Drata index should shrink if retirements processed).",
                "Jul 23-25: Finalize evidence package for Jul 28 Stage 1 audit.",
            ],
        },
        "actionsTakenThisDay": [
            "09:30 PT: SecOps captured this RBR-606 Day 11 snapshot (fleet + pilot) via direct Drata API call -- Phase 4 kickoff.",
            "Reconciled day-over-day delta vs RBR-603 (Jul 20).",
            ...
            "Emitted Phase 4 evidence readiness assessment (A.8.1, A.8.7, A.8.8).",
            "Posted Phase 4 kickoff status to RBR-606.",
        ],
        "nextDayActions": [
            "Jul 22 (Wed): Phase 4 Day 2 — continue evidence assembly.",
            "Jul 22: IT processes first batch of Drata retirement tickets.",
            "Jul 23-25: Finalize evidence package for Jul 28 Stage 1 audit.",
        ],
    }
    ...
    monitor["rollout"] = {
        "phase": "Phase 4 — Evidence verification (Day 11)",
        "owningIssueId": "f0a9fd8c-57f5-44b1-984e-c86cb4da5836",
        "owningIssueIdentifier": "RBR-606",
    ...
    summary = {
        ...
        "phase": "Phase 4 evidence verification kickoff (Day 11)",
