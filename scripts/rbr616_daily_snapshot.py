#!/usr/bin/env python3
"""
RBR-616 daily 09:30 PT snapshot (Jul 25, Day 15).

Day 15 — Saturday Jul 25, Phase 4 Day 5.
This is the FINAL evidence readiness gate before the Jul 28 Stage 1 audit.
Evidence assembly continues; check if IT processed any Drata retirement tickets.
This is the fifth daily snapshot in the Phase 4 evidence window (Jul 21–28).

Per playbook §3.3, today is Phase 4 Day 5. Actions:
  - Capture fleet + pilot snapshot (same format as RBR-614).
  - Reconcile against RBR-614 (Jul 24) for day-over-day delta.
  ...
  - Emit Phase 4 Day 5 status: evidence assembly continues, 3 days to Jul 28 audit.
  - FINAL evidence readiness gate — assess readiness for Stage 1 audit.
...
def main():
    ...
    if pilot_pct >= 90:
        ...
        gate = "ABORT (Phase 1 EOD Jul 11 report; no change at 09:30 PT Jul 25)"

    outstanding_installs = sum(1 for d in devices if d.get("sourceType") != "AGENT")

    # ------------------------------------------------------------------
    # Data reconciliation vs RBR-614 (last snapshot, Jul 24)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Residual hand-off (Phase 4 Day 5 — IT continues retirement)
    # ------------------------------------------------------------------
    touched_ids = set()
    ...
    # ------------------------------------------------------------------
    # FINAL evidence readiness gate assessment
    # ------------------------------------------------------------------
    ...
    snapshot = {
        ...
        "capturedBy": "secops",
        "capturedFor": "RBR-616 daily 09:30 PT snapshot \u2014 Day 15 (Phase 4 Day 5). FINAL evidence readiness gate before Jul 28 audit.",
        "scope": "fleet+pilot",
        ...
        "topIssues": [
            f"Day 15 \u2014 Phase 4 Day 5 FINAL evidence readiness gate. Cumulative cascade efficacy: {total_moved}/{total_cascade_targets} across 6 cascade days (unchanged). Device-side staleness is the bottleneck, not cascade architecture.",
            f"Fleet strict-compliant count {fleet_compliant}/{baseline_total} ({pct_compliant}%); delta vs Jul 24: agent {delta_with_agent:+d}, compliant {delta_compliant:+d}.",
            ...
            f"FINAL READINESS VERDICT: {readiness_verdict}. {readiness_verdict_detail}",
        ],
        "dataReconciliation": {
            "priorSnapshot": "2026-07-24 (RBR-614 daily 09:30 PT Day 14)",
            ...
            "note": f"Day-over-day delta vs RBR-614 (Jul 24). Phase 4 Day 5 FINAL gate. {retirement_note}",
        },
        "cumulativeCascadeEfficacy": {
            ...
            "verdict": (
                ...
                "bottleneck, not cascade architecture. Phase 4 Day 5 FINAL gate: evidence assembly "
                "for agent-installed devices + monitor IT Drata retirement progress."
            ),
        },
        "residualHandOff": {
            "phase": "Phase 4 Day 5 \u2014 FINAL gate: IT continues manual Drata retirement",
            ...
            "note": (
                ...
                f"Phase 4 Day 5 FINAL gate: {retirement_note}"
            ),
        },
        "phase4EvidenceReadiness": {
            ...
            "recommendation": (
                ...
                f"FINAL GATE: {days_remaining} days remain until Jul 28 Stage 1 audit. "
                f"Readiness verdict: {readiness_verdict}."
            ),
        },
        "dayNCascadePlan": {
            "day": 5,
            "phase": "Phase 4 evidence assembly (FINAL gate)",
            "phaseDay": 15,
            "action": (
                "Phase 4 Day 5 FINAL gate \u2014 evidence assembly concludes. Check if IT processed Drata "
                "retirement tickets (Drata index should shrink). SecOps completes A.8.1/A.8.7/A.8.8 "
                "evidence package assessment. No cascade outreach today (all cascade days exhausted). "
                "CISO must decide: proceed with partial evidence or delay audit."
            ),
            "scheduledAt": now.isoformat().replace("+00:00", "Z"),
            "owner": "secops (evidence assembly) + IT (device retirement) + CISO (go/no-go decision)",
            "nextDayActions": [
                "Jul 26 (Sun): Phase 4 Day 6 \u2014 if CISO approves, finalize evidence package.",
        ...
        "actionsTakenThisDay": [
            f"09:30 PT: SecOps captured this RBR-616 Day 15 snapshot (fleet + pilot) via direct Drata API call \u2014 Phase 4 Day 5 FINAL gate.",
            "Reconciled day-over-day delta vs RBR-614 (Jul 24).",
            ...
            "Emitted Phase 4 Day 5 evidence readiness assessment (A.8.1, A.8.7, A.8.8).",
            f"FINAL READINESS VERDICT: {readiness_verdict}. {readiness_verdict_detail}",
        ],
        "nextDayActions": [
            "Jul 26 (Sun): Phase 4 Day 6 \u2014 if CISO approves, finalize evidence package.",
    ...
    monitor["rollout"] = {
        "phase": f"Phase 4 \u2014 Evidence assembly (Day {15}, FINAL gate)",
        "owningIssueId": "9b71d431-135f-4035-be8e-bbd208fe7e71",
        "owningIssueIdentifier": "RBR-616",
    ...
    summary = {
        ...
        "phase": "Phase 4 evidence assembly Day 5 FINAL gate (Day 15)",
