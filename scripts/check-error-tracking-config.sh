...
echo "Breadcrumbs:"
if grep -rn "maxBreadcrumbs" "$ROOT_DIR/server/src" "$ROOT_DIR/packages" "$ROOT_DIR/cli/src" 2>/dev/null | grep -v "\.test\." | grep -v "__tests__" | grep -v "node_modules" | grep -v "check-error-tracking" | grep -E 'maxBreadcrumbs\s*:\s*[6-9][0-9]|[0-9]{3,}' | head -1; then
