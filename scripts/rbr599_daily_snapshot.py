#!/usr/bin/env python3
"""
RBR-599 daily 09:30 PT snapshot (Jul 19, Day 9).

Phase 3 closed EOD Jul 18 (per playbook §5). Today is the first
post-Phase-3 business day. Phase 4 (RBR-93) doesn't start until
Jul 21 (Monday). This is a "holding pattern" snapshot — no fresh
cascade action, just continuing the daily monitor cadence to
capture any organic movement in the fleet.

Per playbook §3.3, today falls in the gap between Phase 3 close
and Phase 4 start. Actions:
  - Capture fleet + pilot snapshot (same format as RBR-559).
  - Reconcile against RBR-559 (Jul 18) for day-over-day delta.
...
def main():
    ...
    if pilot_pct >= 90:
        ...
        gate = "ABORT (Phase 1 EOD Jul 11 report; no change at 09:30 PT Jul 19)"

    outstanding_installs = sum(1 for d in devices if d.get("sourceType") != "AGENT")

    # ------------------------------------------------------------------
    # Data reconciliation vs RBR-559 (last snapshot, Jul 18)
    # ------------------------------------------------------------------
    ...
    snapshot = {
        ...
        "capturedBy": "secops",
        "capturedFor": "RBR-599 daily 09:30 PT snapshot -- Day 9 (post-Phase-3 close, pre-Phase-4 start). Holding pattern: no cascade action today. Captures organic fleet movement + continues cumulative cascade efficacy readout.",
        "scope": "fleet+pilot",
        ...
        "topIssues": [
            f"Day 9 holding pattern -- Phase 3 closed EOD Jul 18; Phase 4 starts Jul 21. Cumulative cascade efficacy: {total_moved}/{total_cascade_targets} across all 6 cascade days (unchanged pattern from Jul 18). Device-side staleness remains the bottleneck.",
            f"Fleet strict-compliant count {fleet_compliant}/{baseline_total} ({pct_compliant}%); delta vs Jul 18: agent {delta_with_agent:+d}, compliant {delta_compliant:+d}.",
        ...
        "dataReconciliation": {
            "priorSnapshot": "2026-07-18T16:37Z (RBR-559 daily 09:30 PT Day 8 = Phase 3 phase-gate report)",
            ...
            "note": "Day-over-day delta vs RBR-559 (Jul 18). Phase 3 closed; no cascade action today.",
        },
        ...
        "residualHandOff": {
            "phase": "Phase 3 closed -> Phase 4 pre-start",
            ...
            "note": (
                ...
                "Phase 4 (RBR-93) starts Jul 21 -- IT should pair each with a manual "
                "Drata retirement ticket to clear the backlog."
            ),
        },
        "dayNCascadePlan": {
            "day": 0,
            "phase": "Holding pattern",
            "phaseDay": 9,
            "action": (
                "Post-Phase-3 close holding pattern. No cascade action today. "
                "Phase 4 (RBR-93) starts Jul 21 with IT ongoing support taking "
            ...
            "nextDayActions": [
                "Jul 20 (Sun): No business-day snapshot.",
                "Jul 21 (Mon): Phase 4 starts -- RBR-93 kickoff; IT takes residual hand-off cohort.",
                "Jul 21: RBR-98 phase-gate report to CISO summarizing cumulative cascade efficacy.",
            ],
        },
        "actionsTakenThisDay": [
            "09:30 PT: SecOps captured this RBR-599 Day 9 snapshot (fleet + pilot) via direct Drata API call -- post-Phase-3 holding pattern.",
            "Reconciled day-over-day delta vs RBR-559 (Jul 18).",
            "Refreshed cumulative cascade efficacy across 6 cascade days: unchanged.",
            "Updated residual hand-off count for Phase 4 queue.",
        ],
        "nextDayActions": [
            "Jul 21 (Mon): Phase 4 (RBR-93) kickoff -- IT ongoing support takes residual hand-off cohort.",
    ...
    summary = {
        ...
        "phase": "Post-Phase-3 holding pattern (Day 9)",
