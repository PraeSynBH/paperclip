#!/usr/bin/env python3
"""
RBR-469 daily 09:30 PT snapshot (Jul 13, Day 3 of Phase 2 Wave 1).
...
Day-3 cascade plan per playbook §3.3:
  Personal 1:1 outreach to top 20 blockers; offer remote-install session.
"""
import json
import os
import sys
...
def main():
    ...
    if pilot_pct >= 90:
        ...
        gate = "ABORT (Phase 1 EOD Jul 11 report; no change at 09:30 PT Jul 13)"
    ...
    # Day-3 cascade: 1:1 outreach to top 20 blockers
    # Sort: no-agent first, then by oldest lastCheckedAt
    outreach_targets = []
    for d in devices:
        last = d.get("lastCheckedAt")
        last_dt = datetime.fromisoformat(last.replace("Z", "+00:00")) if last else None
    ...
    snapshot = {
        ...
        "capturedBy": "secops",
        "capturedFor": "RBR-469 daily 09:30 PT snapshot — Wave 1 Day 3 (morning-after-dept-lead-cascade; pre-1:1 outreach)",
        "scope": "fleet+pilot",
        ...
        "topIssues": [
            f"Wave 1 Day 3 — 1:1 outreach fires at 13:00 PT per playbook §3.3 against top 20 of {len(outreach_targets)} silent/no-agent devices.",
            f"Pilot gate status unchanged from Day 2: {pilot_compliant}/30 compliant (gate >=27/30). ABORT disposition holds; root cause is heartbeat ingestion (RBR-105/106) not install (RBR-410).",
        ...
        "dataReconciliation": {
            "priorSnapshot": "2026-07-12T16:32Z (RBR-455 daily 09:30 PT Day 2)",
            "priorWithAgent": 425,
            "priorCompliant": 87,
            "currentWithAgent": fleet_with_agent,
            "currentCompliant": fleet_compliant,
            "deltaWithAgent": fleet_with_agent - 425,
            "deltaCompliant": fleet_compliant - 87,
            "note": "Compared to RBR-455 last snapshot (Day 2). Positive delta = installs overnight OR devices that re-emerged with fresh heartbeats; negative = agents that went silent and crossed the 48h threshold.",
        },
        "dayNCascadePlan": {
            "day": 3,
            "action": "Personal 1:1 outreach to top 20 blockers; offer remote-install session (per playbook §3.3).",
            "scheduledAt": "2026-07-13T13:00:00-07:00",
            "owner": "secops",
            "targets": top_20,
        ...
        "actionsTakenThisDay": [
            "09:30 PT: SecOps captured this RBR-469 Day 3 snapshot (fleet + pilot) via direct Drata API call.",
            "Pending 13:00 PT: personal 1:1 outreach to top 20 blockers per playbook §3.3 — 1:1 Slack DM or email to the device owner, with a 15-min Zoom offer for remote-install help.",
            "Pending 13:00 PT: spot-check on the IT remote-push ticket (filed in RBR-456) — did any of the top-50 oldest-stalled devices move into the index?",
        ],
        "nextDayActions": [
            "Day 4 (Jul 14 09:30 PT): RBR-469 fires again — measure 1:1 outreach delta.",
            "Day 5 (Jul 14 EOD): Phase-gate Wave 1 metrics report — gate target 90% /fleet or revert per prep-pack §6.",
            "Day 3 EOD: Slack #secops-pilot post with 09:30 PT vs EOD delta and Day-4 plan (handled in cascade child issue).",
        ],
    }
