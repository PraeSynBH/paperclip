#!/usr/bin/env python3
"""
RBR-609 daily 09:30 PT snapshot (Jul 22, Day 12).

Day 12 — Wednesday Jul 22, Phase 4 Day 2.
Evidence verification continues; IT processes first batch of Drata retirement tickets.
This is the second snapshot in the Phase 4 evidence window.

Per playbook §3.3, today is Phase 4 Day 2. Actions:
  - Capture fleet + pilot snapshot (same format as RBR-606).
  - Reconcile against RBR-606 (Jul 21) for day-over-day delta.
  ...
  - Emit Phase 4 Day 2 status: evidence assembly continues.
...
def main():
    ...
    if pilot_pct >= 90:
        ...
        gate = "ABORT (Phase 1 EOD Jul 11 report; no change at 09:30 PT Jul 22)"

    outstanding_installs = sum(1 for d in devices if d.get("sourceType") != "AGENT")

    # ------------------------------------------------------------------
    # Data reconciliation vs RBR-606 (last snapshot, Jul 21)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Residual hand-off (Phase 4 Day 2 — IT continues retirement)
    # ------------------------------------------------------------------
    touched_ids = set()
    ...
    snapshot = {
        ...
        "capturedBy": "secops",
        "capturedFor": "RBR-609 daily 09:30 PT snapshot — Day 12 (Phase 4 Day 2). Evidence verification continues. IT processes first batch of Drata retirement tickets.",
        "scope": "fleet+pilot",
        ...
        "topIssues": [
            f"Day 12 — Phase 4 Day 2 evidence verification. Cumulative cascade efficacy: {total_moved}/{total_cascade_targets} across 6 cascade days (unchanged). Device-side staleness is the bottleneck, not cascade architecture.",
            f"Fleet strict-compliant count {fleet_compliant}/{baseline_total} ({pct_compliant}%); delta vs Jul 21: agent {delta_with_agent:+d}, compliant {delta_compliant:+d}.",
            ...
            f"{by_os['Unknown']['total']} Unknown-OS orphans {'unchanged' if by_os['Unknown']['total'] == 197 else 'changed from 197'} — RBR-108 status review.",
        ],
        "dataReconciliation": {
            "priorSnapshot": "2026-07-21 (RBR-606 daily 09:30 PT Day 11)",
            ...
            "note": f"Day-over-day delta vs RBR-606 (Jul 21). Phase 4 Day 2. {retirement_note}",
        },
        "cumulativeCascadeEfficacy": {
            ...
            "verdict": (
                ...
                "bottleneck, not cascade architecture. Phase 4 Day 2 focus: continue evidence verification "
                "for agent-installed devices + monitor IT Drata retirement progress."
            ),
        },
        "residualHandOff": {
            "phase": "Phase 4 Day 2 — IT continues manual Drata retirement",
            ...
            "note": (
                ...
                f"Phase 4 Day 2: {retirement_note}"
            ),
        },
        "phase4EvidenceReadiness": {
            ...
            "recommendation": (
                ...
                "and manual Drata retirement for stale residuals. 6 days remain until Jul 28 Stage 1 audit."
            ),
        },
        "dayNCascadePlan": {
            "day": 2,
            ...
            "phaseDay": 12,
            "action": (
                "Phase 4 Day 2 — evidence verification continues. IT processes first batch of "
                "Drata retirement tickets. SecOps continues A.8.1/A.8.7/A.8.8 evidence package "
                "assembly. No cascade outreach today (all cascade days exhausted)."
            ),
            "scheduledAt": now.isoformat().replace("+00:00", "Z"),
            "owner": "secops (evidence verification) + IT (device retirement)",
            "nextDayActions": [
                "Jul 23 (Thu): Phase 4 Day 3 — continue evidence assembly; check retirement delta.",
                "Jul 23-25: Finalize evidence package for Jul 28 Stage 1 audit.",
                "Jul 25: Final evidence readiness gate before audit.",
            ],
        },
        "actionsTakenThisDay": [
            "09:30 PT: SecOps captured this RBR-609 Day 12 snapshot (fleet + pilot) via direct Drata API call -- Phase 4 Day 2.",
            "Reconciled day-over-day delta vs RBR-606 (Jul 21).",
            ...
            "Emitted Phase 4 Day 2 evidence readiness assessment (A.8.1, A.8.7, A.8.8).",
        ],
        "nextDayActions": [
            "Jul 23 (Thu): Phase 4 Day 3 — continue evidence assembly.",
            "Jul 23: Check delta on retired devices (Drata index should shrink if retirements processed).",
            "Jul 23-25: Finalize evidence package for Jul 28 Stage 1 audit.",
        ],
    }
    ...
    monitor["rollout"] = {
        "phase": f"Phase 4 — Evidence verification (Day {12})",
        "owningIssueId": "8f5888d2-bbea-4d97-b207-b300bbae316a",
        "owningIssueIdentifier": "RBR-609",
    ...
    summary = {
        ...
        "phase": "Phase 4 evidence verification Day 2 (Day 12)",
