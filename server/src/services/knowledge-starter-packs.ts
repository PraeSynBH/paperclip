...
function loadPacks(): KnowledgeStarterPack[] {
  let files: string[];
  try {
    files = readdirSync(PACKS_DIR).filter((f: string) => f.endsWith(".json"));
  } catch (err) {
    // Starter pack content is added in a later phase; a missing/empty data
    // directory must not block server boot or the company-templates route.
    logger.warn(
      { err, dir: PACKS_DIR },
      "Knowledge starter packs data directory unavailable; serving empty pack list",
    );
    return [];
  }
  ...
  for (const file of files) {
    try {
      ...
      logger.error({ err, file }, "Failed to load knowledge starter pack");
    }
  }
...
export function knowledgeStarterPackService(db: Db): KnowledgeStarterPackService {
  ...
  async function installPack(
    ...
    if (!pack) {
      throw new Error(
        `Starter pack '${packKey}' not found. Available packs: ${packs.map((p) => p.key).join(", ")}`,
      );
    }
    ...
    for (const doc of pack.documents) {
      ...
      try {
        ...
        await knowledgeSvc.publish(companyId, created.id, {
          changeDescription: "Initial publication from starter pack",
        });
