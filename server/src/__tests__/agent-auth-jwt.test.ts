...
describe("agent local JWT", () => {
  ...
  // is the 7th positional arg (after the `keyScope` param), so tests below
  // pass the default standard scope explicitly to reach it.
  describe("run-derived TTL (RBR-1035 AC1)", () => {
    const standardScope = { kind: "standard" as const };
