...
// ---------------------------------------------------------------------------
// Funnel event trackers
// ---------------------------------------------------------------------------

describe("funnel event trackers", () => {
  it("trackSignupCompleted fires signup.completed with correct properties", async () => {
    process.env[API_KEY_ENV] = "phc_test_key";
    process.env[HOST_ENV] = "http://localhost:8000";
    const { trackSignupCompleted } = await importFreshPosthog();

    trackSignupCompleted("comp-1", "user-1", 14);
  ...
  it("trackTrialStarted fires trial.started with correct properties", async () => {
    process.env[API_KEY_ENV] = "phc_test_key";
    process.env[HOST_ENV] = "http://localhost:8000";
    const { trackTrialStarted } = await importFreshPosthog();

    trackTrialStarted("comp-1", 14, "Trial");
  ...
  it("trackOnboardingSeedApplied fires onboarding.seed_applied with correct properties", async () => {
    process.env[API_KEY_ENV] = "phc_test_key";
    process.env[HOST_ENV] = "http://localhost:8000";
    const { trackOnboardingSeedApplied } = await importFreshPosthog();

    trackOnboardingSeedApplied("comp-1", "rev-1", true, true, false);

    expect(mockCapture).toHaveBeenCalledWith({
      distinctId: "comp-1",
      event: "onboarding.seed_applied",
      properties: {
        company_id: "comp-1",
        revision: "rev-1",
        has_mission: true,
        has_agent: true,
        has_first_task: false,
      },
    });
  });

  it("trackApprovalCreated fires value_event.approval_created with correct properties", async () => {
    process.env[API_KEY_ENV] = "phc_test_key";
    process.env[HOST_ENV] = "http://localhost:8000";
    const { trackApprovalCreated } = await importFreshPosthog();

    trackApprovalCreated("comp-1", "approval-1", "hire_agent");

    expect(mockCapture).toHaveBeenCalledWith({
      distinctId: "comp-1",
      event: "value_event.approval_created",
      properties: {
        company_id: "comp-1",
  ...
  it("trackDocumentCreated fires value_event.document_created with correct properties", async () => {
    process.env[API_KEY_ENV] = "phc_test_key";
    process.env[HOST_ENV] = "http://localhost:8000";
    const { trackDocumentCreated } = await importFreshPosthog();

    trackDocumentCreated("comp-1", "doc-1", "plan");

    expect(mockCapture).toHaveBeenCalledWith({
      distinctId: "comp-1",
      event: "value_event.document_created",
      properties: {
        company_id: "comp-1",
        document_id: "doc-1",
        document_key: "plan",
  ...
  it("all funnel trackers are no-ops when PostHog is not configured", async () => {
    const {
      trackSignupCompleted,
      trackTrialStarted,
      trackOnboardingSeedApplied,
      trackApprovalCreated,
      trackDocumentCreated,
    } = await importFreshPosthog();

    trackSignupCompleted("c1", "u1", 14);
    trackTrialStarted("c1", 14, "Trial");
    trackOnboardingSeedApplied("c1", "r1", true, false, false);
    trackApprovalCreated("c1", "a1", "standard");
    trackDocumentCreated("c1", "d1", "plan");
