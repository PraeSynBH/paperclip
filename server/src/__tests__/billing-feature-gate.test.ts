...
describeEmbeddedPostgres("billing feature gating (checkFeatureAccess / requireFeature)", () => {
  ...
  beforeAll(async () => {
    ...
    const now = new Date();
    const periodStart = new Date(now.getTime() - 7 * 86400_000); // started 7 days ago
    const periodEndFuture = new Date(now.getTime() + 23 * 86400_000); // ends in 23 days
    const periodEndPast = new Date(now.getTime() - 1 * 86400_000); // ended yesterday
    ...
    for (let i = 0; i < cids.length; i++) {
      ...
      await db.insert(companies).values({
        id,
        name: `FeatureGate Co ${id.slice(0, 6)}`,
        status: "active",
