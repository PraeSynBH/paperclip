...
describeEmbeddedPostgres("Stripe billing E2E flow (service layer)", () => {
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  // 14. Billing portal link
  // ─────────────────────────────────────────────────────────────────────────────
  (hasStripeKeys ? it : it.skip)("creates billing portal session link", async () => {
    const { getStripeClient } = await import("../services/billing.js");
    const stripe = getStripeClient();
    ...
    await db.insert(companies).values({
      id: tempCompanyId,
      name: `Portal-Test ${tempCompanyId.slice(0, 6)}`,
      status: "active",
      issuePrefix: "PRT",
      updatedAt: now,
    });
    ...
    const portalResult = await billing.getBillingPortalLink(tempCompanyId);
    expect(portalResult).toBeDefined();
    expect(portalResult.url).toBeTruthy();
    expect(portalResult.url).toContain("billing.stripe.com");
  });

  // ─────────────────────────────────────────────────────────────────────────────
  // 15. Trial will end handler
  // ─────────────────────────────────────────────────────────────────────────────
  it("handleTrialWillEnd logs impending trial expiry and updates trialEnd", async () => {
    const tempCompanyId = randomUUID();
    const now = new Date();
    await db.insert(companies).values({
      id: tempCompanyId,
      name: `Trial-End-Test ${tempCompanyId.slice(0, 6)}`,
      status: "active",
      issuePrefix: "TWE",
      updatedAt: now,
    });
    ...
    // Create a subscription with a trial end
    const [tier] = await db
      .select()
    ...
    // Create a synthetic Stripe customer for the temp company
    const [cust] = await db
      .insert(stripeCustomers)
      .values({
        companyId: tempCompanyId,
        stripeCustomerId: `cus_test_trial_${randomUUID().slice(0, 8)}`,
      })
      .returning();
    ...
    // Call handleTrialWillEnd
    await billing.handleTrialWillEnd({
    ...
    // Verify the trial end was updated
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  // 16. Post-trial grace period handling
  // ─────────────────────────────────────────────────────────────────────────────
  it("handlePostTrialStatus enters grace period when trial ends without payment", async () => {
    const tempCompanyId = randomUUID();
    const now = new Date();
    await db.insert(companies).values({
      id: tempCompanyId,
      name: `Grace-Test ${tempCompanyId.slice(0, 6)}`,
      status: "active",
      issuePrefix: "GRC",
      updatedAt: now,
    });
    ...
    // Trial ended yesterday (past)
    const trialEndDate = new Date(Date.now() - 1 * 24 * 60 * 60 * 1000);

    const [cust] = await db
      .insert(stripeCustomers)
      .values({
        companyId: tempCompanyId,
        stripeCustomerId: `cus_test_grace_${randomUUID().slice(0, 8)}`,
      })
      .returning();
    ...
    // Simulate the subscription update from Stripe after trial ends
    const stripeSubId = `sub_grace_${randomUUID().slice(0, 8)}`;
    await db
      .update(companySubscriptions)
      .set({ stripeSubscriptionId: stripeSubId })
      .where(eq(companySubscriptions.id, sub.id));

    // Call handlePostTrialStatus with an "incomplete" status (no payment method)
    await billing.handlePostTrialStatus({
      id: stripeSubId,
      metadata: {
        paperclipCompanyId: tempCompanyId,
      },
      status: "incomplete",
      current_period_start: Math.floor((now.getTime() - 30 * 24 * 60 * 60 * 1000) / 1000),
      current_period_end: Math.floor(now.getTime() / 1000),
      items: { data: [] },
      cancel_at_period_end: false,
    } as any);

    // Verify the subscription is now in grace_period status
    const updated = await db
      .select()
      .from(companySubscriptions)
      .where(eq(companySubscriptions.id, sub.id))
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  // 17. Grace period → expired after grace period elapses
  // ─────────────────────────────────────────────────────────────────────────────
  it("handlePostTrialStatus marks subscription as expired after grace period", async () => {
    const tempCompanyId = randomUUID();
    const now = new Date();
    await db.insert(companies).values({
      id: tempCompanyId,
      name: `Expired-Test ${tempCompanyId.slice(0, 6)}`,
      status: "active",
      issuePrefix: "EXP",
      updatedAt: now,
    });
    ...
    // Trial ended 14 days ago (past the 7-day grace period)
    ...
    const [cust] = await db
      ...
      .values({
        companyId: tempCompanyId,
        stripeCustomerId: `cus_test_expired_${randomUUID().slice(0, 8)}`,
      })
      .returning();
    ...
    const stripeSubId = `sub_expired_${randomUUID().slice(0, 8)}`;
    await db
      .update(companySubscriptions)
      .set({ stripeSubscriptionId: stripeSubId })
      .where(eq(companySubscriptions.id, sub.id));

    // Call handlePostTrialStatus — grace period has elapsed
    await billing.handlePostTrialStatus({
      id: stripeSubId,
      metadata: {
        paperclipCompanyId: tempCompanyId,
    ...
    // Verify the subscription is now expired (data preserved)
    const updated = await db
      .select()
      .from(companySubscriptions)
      .where(eq(companySubscriptions.id, sub.id))
