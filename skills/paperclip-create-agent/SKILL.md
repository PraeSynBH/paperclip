...
```sh
scripts/pc-api.sh get "/api/approvals/<approval-id>"

scripts/pc-api.sh post "/api/approvals/<approval-id>/comments" '{"body":"## CTO hire request submitted\n\n- Approval: [<approval-id>](/approvals/<approval-id>)\n- Pending agent: [<agent-ref>](/agents/<agent-url-key-or-id>)\n- Source issue: [<issue-ref>](/issues/<issue-identifier-or-id>)\n\nUpdated prompt and adapter config per board feedback."}'
```

If the approval already exists and needs manual linking to the issue:

```sh
scripts/pc-api.sh post "/api/issues/<issue-id>/approvals" '{"approvalId":"<approval-id>"}'
```

After approval is granted, run this follow-up loop:

```sh
scripts/pc-api.sh get "/api/approvals/$PAPERCLIP_APPROVAL_ID"

scripts/pc-api.sh get "/api/approvals/$PAPERCLIP_APPROVAL_ID/issues"
```
