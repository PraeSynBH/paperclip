...
export const REDACTION_PATTERNS: readonly RedactionPattern[] = [
  ...
  { class: "email_idn", pattern: /[\p{L}\p{N}._%+\-]+@[\p{L}\p{N}.\-]+\.[\p{L}]{2,}/giu, replacement: "[REDACTED:email_idn]" },
  ...
  { class: "phone_us", pattern: /(?<!\d)(?:\+1|1)?[\s.\-]*\(?[2-9]\d{2}\)?[\s.\-]*\d{3}[\s.\-]*\d{4}(?!\d)/g, replacement: "[REDACTED:phone_us]" },
