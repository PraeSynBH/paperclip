...
**Status:** Final — Added pricing funnel GA4 events to Business Events section (VOY-1733). Sanitization now operates on a clone of the error (P2-1 / b6c96c2f55); original error objects are never mutated. v1.5.0's in-place-mutation description superseded.
**Applies to:** VOY-999 / VOY-1007 / VOY-1015 / VOY-1029 / VOY-1420 / VOY-1428 / VOY-1430 / VOY-1456 (P2-1) / VOY-1733
...
### Pricing Funnel Analytics (GA4)

Pricing funnel conversion events are sent through **Google Analytics 4** (not PostHog) using the client-side `gtag` function on the pricing page. These events measure user progression through the subscription conversion flow. The server-side GA4 service (`ga4-analytics.ts`) provides supplemental server-side events.

Unlike PostHog business events (which can be queried via PostHog API), GA4 events are routed to the Google Analytics property configured via `GA4_MEASUREMENT_ID`. The support team should verify event delivery using the GA4 Debug endpoint or server-side logs.

#### Instrumented Pricing Funnel Events (GA4)

| Event Name | Source | Trigger | Properties |
|---|---|---|---|
| `cta_clicked` | Pricing page (client-side `gtag`) | User clicks a CTA button on a pricing tier card | `tier_id`, `tier_name`, `billing_period`, `experiment_variant`, `company_id` |
| `checkout_started` | Pricing page (client-side `gtag`) | Stripe checkout session is created after CTA click | `tier_id`, `billing_period`, `experiment_variant`, `company_id` |
| `begin_checkout` | Server-side billing service (`billing.ts`) | Checkout session creation — server-side supplement | `company_id`, `tier_id`, `billing_period`, `value`, `currency` |
| `billing_period_toggle` | Pricing page (client-side `gtag`) | User toggles between monthly and yearly billing | `period`, `company_id` |
| `subscription_cancellation_started` | Pricing page (client-side `gtag`) | User confirms cancellation from subscription management | `company_id` |

**Note:** These events are sent to **GA4**, not PostHog. They cannot be queried via the PostHog API. To verify delivery, use the GA4 Debug endpoint or server-side logs (see [GA4 Analytics Service](../documentation/ga4-analytics.md) for configuration).

### Debugging Business Events

#### PostHog Event Debugging
```bash
...
#### Pricing Funnel GA4 Debugging
```bash
# Enable GA4 debug mode to validate pricing funnel events
# Set GA4_DEBUG=true in environment and re-deploy
# Events will be sent to the GA4 Measurement Protocol debug endpoint
# Check server logs for GA4 validation errors

# Verify checkout_started event payload (requires GA4_DEBUG=true)
# Check server logs for: "GA4 responded with" or "GA4 request failed"
# Expected: statusCode 201 or 204 on success; 4xx with validation details on failure

# Client-side gtag events can be verified using GA4 DebugView in the GA4 web console
# (Configure: GA4 Admin > Streams > your web stream > DebugView)
# Open your browser's GA4 DebugView extension or use the gtag debugger
```
...
- **Pricing funnel events not appearing in GA4** — Check that `GA4_ENABLED=true`, `GA4_MEASUREMENT_ID`, and `GA4_API_SECRET` are set. Enable `GA4_DEBUG=true` to validate event payloads via the debug endpoint. Note that GA4 real-time reports have a 24-48 hour delay for Measurement Protocol events — use DebugView for immediate verification.
- **checkout_started fires but no begin_checkout server event** — This is expected if `GA4_ENABLED` is not `true` on the server (server-side events are gated separately from client-side gtag). Check both server and client GA4 configuration independently.
- **cta_clicked not firing** — Verify the user has a company selected (pricing page requires company context). Check browser console for gtag errors. The `experiment_variant` property may be `null` if the experiment is disabled — this is valid.
- **experiment_variant property is always null** — The `GET /api/companies/:id/billing/experiment-variant` endpoint may not be configured (experiment disabled or config missing). Check `PRICING_EXPERIMENT_CONFIG` environment variable. The experiment variant is only assigned after the first pricing page visit with the experiment enabled.
