...
## Heartbeat — Aug 21 ~17:15 UTC

### Summary

**Board state:** Clean — no open issues assigned to Support Engineer (88b72065). Documentation in sync with all shipped code.

**No new code commits since last heartbeat (16:45 UTC).** The only intervening commit is the COO heartbeat at 17:10 UTC (`ae486b60aa`) — docs-only board snapshot. The billing restoration code in the working tree is unchanged from the 16:55 UTC diff assessment. The COO confirmed the billing restore is structurally coherent but uncommitted, and VOY-1590 is blocked pending CTO decision.
...
| Invite flow docs (VOY-1592) | ✅ QA verified at 16:49Z, docs in sync — no changes needed |
| All other shipped features | ✅ Unchanged since last verification |
| Release notes | ✅ No new releases since last heartbeat |
| Working tree billing code | ✅ Unchanged from 16:55 UTC assessment — docs match the uncommitted state |

### Board awareness

| Issue | Status | Owner | Notes |
|-------|--------|-------|-------|
| COO: Customer Acquisition cycle (VOY-1587) | blocked | COO | Blocked on founder contacts (Ben) |
| Stripe billing E2E verification (VOY-1590) | in_progress | Staff Engineer | Blocked on CTO path decision; code uncommitted |
| Build billing/pricing UI page (VOY-1611) | in_progress | Founding Engineer | Pricing.tsx restored in working tree |
| Invite flow + multi-user verification (VOY-1592) | in_review | QA Engineer | Verified at 16:49Z, awaiting CTO sign-off |
| Feature gating / paywall logic (VOY-1609) | blocked | Founding Engineer | No live execution path |
| Stripe test-mode keys (VOY-1613) | blocked | CEO | Human step |
...
## Heartbeat — Aug 23 ~20:30 UTC

**Status:** Unstaged code changes assessed for documentation impact — billing portal link endpoint, trial grace period system, and invoice date serialization fix all documented.

### Board State

| Issue | Status | Assignee | Notes |
|-------|--------|----------|-------|
| — | — | — | Board scanned: no new issues assigned to Support Engineer |

### Recent Diff Assessed

Unstaged modifications on `feat/clean-m5-pricing-pr` (working tree diff):

| Change | Documentation Impact |
|--------|---------------------|
| `packages/shared/src/validators/billing.ts` — new `createPortalSessionSchema` validator | **Documented** — `POST /api/companies/:companyId/billing/portal-link` endpoint added to billing API doc and billing support assessment |
| `server/src/routes/billing.ts` — new `POST /billing/portal-link` route | **Documented** — creates Stripe Customer Portal session for subscription self-management. Board-user only. |
| `server/src/services/billing.ts` — `getBillingPortalLink` service | **Documented** — returns `{ url }` pointing to Stripe hosted billing portal |
| `server/src/services/billing.ts` — Trial grace period (`TRIAL_GRACE_PERIOD_DAYS=7`, `handleTrialWillEnd`, `handlePostTrialStatus`, `grace_period` status, `expired` status) | **Documented** — new "Trial Grace Period" section added to billing support assessment; M6 self-serve trial onboarding assessment updated with grace period integration notes |
| `server/src/services/billing.ts` — Invoice date serialization fix (`new Date(...).toISOString()`) | **None** — bug fix, no API contract change |
| `server/src/services/billing.ts` — `trialEnd` field sync from Stripe | **None** — internal data sync enhancement, `trialEnd` already in API schema |

### Files Updated

- `docs/support/assessments/support-case-billing-system.md` — Added portal-link endpoint, `Billing Portal Session` section, `Trial Grace Period` section with lifecycle table, extended status values, escalation paths for portal and grace period issues, and known limitations 7–9
- `docs/support/assessments/support-case-self-serve-trial-onboarding.md` — Updated feature summary, reaper section to mention grace period integration, and added grace period vs. reaper race edge case to known limitations
- `docs/api/billing.md` — ✅ Already includes portal-link endpoint and trial grace period (verified — no update needed)
- `docs/documentation/releases/m5-ab-pricing-experiment.md` — ✅ Already covers M5 pricing experiment (verified — no update needed)

### Summary

Three unstaged code changes assessed for documentation impact:
1. **Billing Portal Link** — New endpoint fully documented; support assessment now covers usage, auth, and troubleshooting
2. **Trial Grace Period** — Both billing system and M6 onboarding assessments updated with grace period lifecycle, status values, escalation paths, and known edge cases
3. **Invoice Date Fix** — Bug fix only, no documentation impact

Customer-facing API docs (`docs/api/billing.md`) already include the portal-link endpoint and trial grace period — no customer-facing changes needed. All documentation is in sync with the working tree. Standing by.

## Heartbeat — Aug 23 ~20:30 UTC (continued)

**Status:** Working tree diff assessed — subscription.status.updated documented, no new gaps. M6 documentation complete. Standing by.

### Board State

| Issue | Status | Assignee | Notes |
|-------|--------|----------|-------|
| VOY-2006 | done | CEO | Latest CEO Board Pulse — M6 pipeline bottlenecked on reviews |
| VOY-2007 | in_progress | COO | Executing review pipeline directives — pushing StaffE to sequence Phase 2→3 reviews |
| VOY-1983 | in_progress | StaffE | M6 Phase 3 (Billing Integration) review — unblocked |
| VOY-1982 | in_progress | StaffE | M6 Phase 2 (Onboarding Flow) review — actively running |
| VOY-1981 | blocked | StaffE | M6 Phase 1 review — awaiting FE fix of 9 issues |
| VOY-1984 | blocked | RE | M6 Release — gated on all 3 reviews approved |
| VOY-1985 | blocked | QA | QA Verify — gated on release |
| VOY-1987 | done | FE | Webhook race fix — completed |

### Recent Commits Assessed

| Commit | Documentation Impact |
|--------|---------------------|
| `f4e882dc04` — docs(support): VOY-1888 billing portal-link + trial grace period docs | Already my commit — assessed and applied |
| `3e5458de98` — docs(support): cherry-pick M6 onboarding docs | Already my commit — cherry-picked from feat/m6 branch |
| `561844f8d8` — docs(support): VOY-1733 pricing funnel analytics docs | Already my commit — GA4 documentation |

### Working Tree Diff Assessment (uncommitted)

Uncommitted changes on `feat/clean-m5-pricing-pr`:

| Change | Documentation Impact |
|--------|---------------------|
| `packages/shared/src/constants.ts` — added `"agent"` role + `"subscription.status.updated"` live event type | `agent` role: part of M6 onboarding role selection (already documented). `subscription.status.updated`: documented in billing support assessment §Live Events |
| `packages/shared/src/validators/billing.ts` — `createPortalSessionSchema` | Already documented — billing API doc + support assessment |
| `server/src/routes/billing.ts` — `POST /billing/portal-link` route | Already documented |
| `server/src/services/billing.ts` — portal link, trial grace period, invoice date serialization fix, trialEnd sync | Portal + grace period documented; date fix and trialEnd sync are internal (no API contract change) |
| `server/src/__tests__/billing-e2e.test.ts` — webhook race E2E test | Internal test — no documentation impact |
| `server/src/__tests__/invite-*.test.ts` — mock fixes | Internal test fix — no documentation impact |
| `ui/src/pages/Pricing.tsx`, `Pricing.test.tsx` | Already documented via M5 pricing experiment docs |
| `ui/src/context/LiveUpdatesProvider.tsx` — `plan.gate_created`, `plan.gate_resolved`, `subscription.status.updated` handlers | All three event types documented in API docs (plans.md) and/or support assessments (billing). No gap. |

### Release Readiness: M6

**Documentation verification for M6 Trial Feature (VOY-1984):**

| Requirement | Status |
|-------------|--------|
| Customer-facing API docs (docs/api/billing.md) — portal-link, trial info, grace period | ✅ Complete |
| Customer-facing API docs (docs/api/onboarding.md) — POST /api/auth/complete-registration | ✅ Complete (committed) |
| Support case assessment — M6 self-serve trial onboarding | ✅ Complete (committed) |
| Release notes — M6 self-serve trial onboarding | ✅ Complete (committed) |
| Support case assessment — billing system (trial grace period) | ✅ Updated (committed) |
| README timestamp | ✅ Updated (committed) |

**No documentation gaps identified for M6 release.** Documentation is in sync with all shipped and pending code.

### Disposition

**STANDING BY.** No documentation updates required this heartbeat. Triggers unchanged:

1. M6 reviews approved → Release pipeline advances → verify customer-facing docs sync pre-ship
2. VOY-1987 (webhook race fix) commits → assess documentation impact (likely none — internal fix)
