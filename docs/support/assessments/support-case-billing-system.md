...
2. **P1: Race in handleSubscriptionUpdated / handleCheckoutSessionCompleted** — ✅ **FIXED** (committed `1fb17b8f18`, then updated in `3885b6b5f0`). Uses `INSERT ... ON CONFLICT (company_id) DO UPDATE SET` — concurrent Stripe events are idempotent. The conflict target was changed from `stripe_subscription_id` to `company_id` in `3885b6b5f0` to correctly handle trial-to-paid conversion: the trial row has `stripe_subscription_id = NULL`, and SQL NULL comparison semantics would cause `ON CONFLICT (stripe_subscription_id)` to miss the match.
