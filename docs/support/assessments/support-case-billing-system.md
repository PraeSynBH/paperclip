...
The pricing page sends conversion tracking events to **Google Analytics 4** to measure user progression through the subscription funnel. These events are sent from two sources:

| Source | Technology | Events |
|--------|-----------|--------|
| **Pricing page UI** (client-side) | `gtag` (Google tag) in browser | `cta_clicked`, `checkout_started`, `billing_period_toggle`, `subscription_cancellation_started` |
| **Billing service** (server-side) | GA4 Measurement Protocol via `ga4-analytics.ts` | `begin_checkout` |

**Client-side events** fire on user actions (CTA click, billing period toggle, cancel confirmation). They require the GA4 web stream to be configured in the GA4 admin console — this is independent of the server-side GA4 configuration.

**Server-side events** fire during checkout session creation. They are gated by `GA4_ENABLED=true` on the server and are sent to the GA4 property configured by `GA4_MEASUREMENT_ID` and `GA4_API_SECRET`.

**GA4 is not a PostHog replacement.** These events go to GA4, not PostHog. Support should check GA4 DebugView for real-time client-side event verification and server logs for server-side event validation. See [GA4 Analytics Service Documentation](../documentation/ga4-analytics.md) for configuration and the [PostHog SOP](../posthog-error-monitoring-triage-sop.md) for pricing funnel troubleshooting guidance.

**Experiment variant metadata:** When the A/B pricing experiment is active, all pricing funnel events include `experiment_variant` (variant A or B) in their properties, enabling conversion analysis by variant. The variant is assigned server-side and fetched via `GET /api/companies/:id/billing/experiment-variant`.
