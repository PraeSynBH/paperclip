...
export function useBackgroundProcesses(
  ...
  //
  // On first mount and after any reconnect, `onopen` fires to clear the grace
  // timer and any polling interval. On error, we start polling as a fallback.
  // When SSE reconnects (browser auto-reconnect), `onopen` fires again and
  // clears the polling that `onerror` started — preventing dual-polling.
  useEffect(() => {
    if (!companyId) return;
    ...
    // Shared handler to cancel grace timer and tear down polling.
    // Called on initial connect AND on reconnect after error.
    const clearGraceAndPolling = () => {
    ...
    try {
      ...
      // Fires on initial connect and every browser auto-reconnect after error.
      // This is our primary signal that SSE is alive.
      es.onopen = clearGraceAndPolling;
      ...
      es.onerror = () => {
        // SSE dropped — if we don't have polling yet, start it as a fallback.
        // When the browser auto-reconnects, `onopen` above will clear it.
        if (!pollTimerRef.current) {
          pollTimerRef.current = setInterval(refresh, 5000);
        }
      };
